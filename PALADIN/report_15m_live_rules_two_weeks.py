#!/usr/bin/env python3
"""
Evaluate each **live** ``timeframe == "15m"`` rule in ``shaman_v1_rules.json`` on the trailing
**N×7 days** of Binance BTCUSDT 15m OHLCV (next-bar R/G vs rule ``pred``).

PnL model (per rule, isolated): **$1** per fill by default; win **+1**, loss **-1**;
doji next bar excluded from hit count.

Writes markdown + CSV under ``exports/btc_binance_klines/`` (filenames include week count; for
``--weeks 4`` also writes ``REPORT_15m_live_rules_last_month.md`` as a copy of the report).

Run from repo root:
  python PALADIN/report_15m_live_rules_two_weeks.py --weeks 2   # ~2 weeks (default)
  python PALADIN/report_15m_live_rules_two_weeks.py --weeks 4   # ~4 weeks (~1 month)
"""
...
BARS_PER_WEEK_15M = 7 * 24 * 4  # 672 fifteen-minute bars per 7d
WARMUP = 220
...
def _fetch_15m(*, symbol: str, total: int, out_path: Path) -> int:
    all_rows: list[list[Any]] = []
    end_time: int | None = None
    while len(all_rows) < total:
        need = min(1000, total - len(all_rows))
        q = f"symbol={symbol}&interval=15m&limit={need}"
...
def main() -> int:
    ...
    ap.add_argument("--weeks", type=int, default=2, help="Trailing calendar weeks of 15m bars (7d = 672 bars each).")
    ap.add_argument("--skip-fetch", action="store_true")
    ap.add_argument("--stake-usdc", type=float, default=DEFAULT_STAKE_USDC)
    args = ap.parse_args()

    weeks = max(1, min(104, int(args.weeks)))
    eval_bars = weeks * BARS_PER_WEEK_15M
    stake = float(args.stake_usdc)
    fetch_n = eval_bars + WARMUP
    kpath = OUT_DIR / f"btcusdt_15m_last{fetch_n}_binance.csv"
    OUT_DIR.mkdir(parents=True, exist_ok=True)

    if not args.skip_fetch:
        got = _fetch_15m(symbol=args.symbol, total=fetch_n, out_path=kpath)
    ...
    fifteen = [r for r in rules if str(r.get("timeframe", "")).strip() == "15m"]
    ...
    if weeks == 2:
        report_stem = "REPORT_15m_live_rules_two_weeks"
    else:
        report_stem = f"REPORT_15m_live_rules_{weeks}weeks"
    ...
    lines = [
        f"# Live 15m SHAMAN rules — trailing {title_suffix} (Binance BTCUSDT 15m)",
        "",
        f"- Generated (UTC): `{stamp}`",
        f"- Window: **{weeks}** × 7d = **{eval_bars}** 15m bars on the close of the series.",
        f"- Klines: `{kpath.name}` (**{n}** rows); eval `t` in `[{t_lo}, {t_hi}]` (**{t_hi - t_lo + 1}** steps).",
        f"- Live 15m rules: **{len(fifteen)}**; with ≥1 fill: **{len(with_hits)}**.",
    ...
    if weeks == 4:
        month_path = OUT_DIR / "REPORT_15m_live_rules_last_month.md"
